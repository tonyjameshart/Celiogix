...
def search_recipes(conn: sqlite3.Connection, query: str, n: int = 10) -> List[Dict[str, Any]]:
    ...
    if not app_id or not app_key:
        raise RuntimeError("Missing Edamam credentials. Set EDAMAM_APP_ID / EDAMAM_APP_KEY or use Settings.")
    params = {"q": query, "app_id": app_id, "app_key": app_key, "health": "gluten-free", "to": max(1, min(50, int(n or 10)))}
