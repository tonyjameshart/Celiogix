...
def _shopping_table(db) -> str:
    ...
    for name in ("shopping_list", "shopping_items"):
        if cur.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name=?", (name,)).fetchone():
            return name
    # Create default
    name = "shopping_list"
    cur.execute(f"""
...
def _map_cols(cols: Set[str]) -> Dict[str, Optional[str]]:
...
def _adapt_payload(m: Dict[str, Optional[str]], payload: Dict[str, Any]) -> Dict[str, Any]:
    out: Dict[str, Any] = {}
...
def _merge_or_insert_shopping(db, payload: Dict[str, Any]) -> None:
    ...
    if m["linked_pantry_id"] and payload.get("linked_pantry_id") is not None:
        wheres.append(f"{m['linked_pantry_id']}=?"); params.append(int(payload["linked_pantry_id"]))
    elif m["name"]:
        wheres.append(f"{m['name']}=?"); params.append(payload.get("name", ""))
        if m["unit"]:
            wheres.append(f"IFNULL({m['unit']},'')=?"); params.append(payload.get("unit", "") or "")
    if m["status"]:
        wheres.append(f"IFNULL({m['status']},'pending') IN ('pending','')")
    where_sql = (" WHERE " + " AND ".join(wheres)) if wheres else ""

    # Fetch existing
    sel_cols = [m["id"] or "id"]; alias_q = None
    if m["quantity"]:
        alias_q = "sl_qty"; sel_cols.append(f"{m['quantity']} AS {alias_q}")
    row = cur.execute(f"SELECT {', '.join(sel_cols)} FROM {table}{where_sql}", tuple(params)).fetchone()

    if row and m["quantity"]:
        try: inc = float(payload.get("quantity") or 1)
        except: inc = 1.0
        new_q = (row[alias_q] or 0) + inc
        cur.execute(f"UPDATE {table} SET {m['quantity']}=? WHERE {m['id']}=?", (new_q, row[m["id"] or "id"]))
        db.commit()
        return
    if row:
        return
    ...
    keys = ", ".join(data.keys()); q = ", ".join(["?"] * len(data))
...
def _threshold_value(threshold: Optional[float], base_amount: Optional[float]) -> float:
    """
...
def sync_menu_consumption(db, up_to_date: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    if not menu_rows:
        return {"processed_entries": 0, "updated_items": 0, "skipped_ingredients": 0, "auto_added": 0}
