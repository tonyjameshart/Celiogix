...
if not _RISK_TERMS:
    _RISK_TERMS = {
        "wheat","barley","rye","malt","spelt","farro","durum","semolina",
        "graham","triticale","seitan","kamut","bulgur","couscous","einkorn",
        "wheat starch","barley malt","brewer's yeast","brewers yeast"
    }
if not _MAYBE_TERMS:
    _MAYBE_TERMS = {
        "oats","oat","oat flour","miso","soy sauce","maltodextrin","natural flavors",
        "beer","ale","stout","vinegar malt",
    }
if not _SAFE_TERMS:
    _SAFE_TERMS = {
        "gluten-free","gluten free","certified gluten-free","certified gluten free",
        "gf","no gluten","wheat-free","wheat free","tamari (gluten-free)","rice","corn",
    }
...
def classify_text(text: str) -> Dict[str, Any]:
    ...
    if risk_hits:
        return {"flag": "RISK", "tags": sorted({h.upper() for h in risk_hits}), "evidence": {"risk_terms": risk_hits}}
    if safe_phrase or safe_hits:
        tags = sorted({("CERTIFIED GF" if safe_phrase else None)} | {h.upper() for h in safe_hits} - {None})
        return {"flag": "SAFE", "tags": tags, "evidence": {"safe_terms": safe_hits, "safe_phrase": safe_phrase}}
    if maybe_hits:
        return {"flag": "UNKNOWN", "tags": [f"CHECK:{h.upper()}" for h in sorted(set(maybe_hits))],
