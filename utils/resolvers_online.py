...
try:
    ...
    def risk_badges(text: str) -> List[str]: return []

from .gtin import normalize, classify

OFF_URL = "https://world.openfoodfacts.org/api/v2/product/{code}.json"
UA = {"User-Agent": "CeliacCulinary/1.0 (+local app)"}

RISK_TERMS = (
    "gluten", "wheat", "barley", "rye", "malt", "triticale", "spelt",
    "麦", "大麦", "ライ麦", "glutenhaltig", "gluten-containing"
)

def _parse_quantity(q: Optional[str]) -> tuple[Optional[float], Optional[str]]:
    """
...
def _infer_gf(product: Dict[str, Any]) -> tuple[Optional[int], List[str]]:
    """
    Return (flag, reasons)
