...
class HealthLogPanel(BasePanel):
    ...
    def build(self) -> None:
        ...
        ttk.Entry(addf, textvariable=self.var_date, width=12).grid(row=r, column=1, pady=4, sticky="w")
        ...
        r += 1
        ttk.Label(addf, text="Items/Consumed").grid(row=r, column=0, padx=(6,6), pady=4, sticky="e")
        ttk.Entry(addf, textvariable=self.var_items, width=60).grid(row=r, column=1, columnspan=5, pady=4, sticky="ew")

        ttk.Label(addf, text="Risk").grid(row=r, column=6, padx=(12,6), pady=4, sticky="e")
        ttk.Combobox(addf, textvariable=self.var_risk, width=8, values=RISK_OPTS, state="readonly").grid(row=r, column=7, pady=4, sticky="w")

        r += 1
        ttk.Label(addf, text="Onset (min)").grid(row=r, column=0, padx=(6,6), pady=4, sticky="e")
        ttk.Spinbox(addf, from_=0, to=720, textvariable=self.var_onset, width=6).grid(row=r, column=1, pady=4, sticky="w")

        ttk.Label(addf, text="Bristol").grid(row=r, column=2, padx=(12,6), pady=4, sticky="e")
        cb_bristol = ttk.Combobox(addf, textvariable=self.var_bristol_label, width=44, state="readonly", values=BRS_VALUES)
        cb_bristol.grid(row=r, column=3, columnspan=3, pady=4, sticky="w")

        ttk.Label(addf, text="Severity (0–10)").grid(row=r, column=6, padx=(12,6), pady=4, sticky="e")
        ttk.Spinbox(addf, from_=0, to=10, textvariable=self.var_sev, width=4).grid(row=r, column=7, pady=4, sticky="w")

        # Symptoms blocks
        r += 1
        grid = ttk.Frame(addf); grid.grid(row=r, column=0, columnspan=5, sticky="w", padx=4, pady=(4,6))
        ...
        for i in range(20): filt.grid_columnconfigure(i, weight=0)
        filt.grid_columnconfigure(19, weight=1)

        ttk.Label(filt, text="From").grid(row=0, column=0, padx=(6,6), pady=6, sticky="e")
        ttk.Entry(filt, textvariable=self.f_from, width=12).grid(row=0, column=1, pady=6, sticky="w")
        ...
        ttk.Button(filt, text="This Week", command=self._this_week).grid(row=0, column=4, padx=(8,0), pady=6, sticky="w")

        ttk.Button(filt, text="Load", command=self.load_rows).grid(row=0, column=5, padx=(14,0), pady=6)

        ttk.Label(filt, text="Min Severity").grid(row=0, column=6, padx=(14,6), pady=6, sticky="e")
        ttk.Spinbox(filt, from_=0, to=10, textvariable=self.f_minsev, width=4).grid(row=0, column=7, pady=6, sticky="w")

        ttk.Label(filt, text="Risk").grid(row=0, column=8, padx=(14,6), pady=6, sticky="e")
        ttk.Combobox(filt, textvariable=self.f_risk, width=8, values=["any"] + RISK_OPTS, state="readonly").grid(row=0, column=9, pady=6, sticky="w")
        ...
        ttk.Button(filt, text="Copy", command=self.copy_visible_to_clipboard).grid(row=0, column=12, padx=(12,0), pady=6)
        ttk.Button(filt, text="Export HTML", command=self.export_html_visible).grid(row=0, column=13, padx=(8,0), pady=6)
    ...
    def clear_form(self) -> None:
        ...
        for v in self.sym_vars.values(): v.set(False)

    # ---------- Filtering / loading ----------
    def _this_week(self) -> None:
        today = _dt.date.today()
        start = today - _dt.timedelta(days=today.weekday())  # Monday
        self.f_from.set(start.isoformat()); self.f_to.set(today.isoformat())
    ...
    def _filters_sql(self) -> Tuple[str, List[Any]]:
        ...
        if f:
            where.append("date >= ?"); args.append(f)
        if t:
            where.append("date <= ?"); args.append(t)
        if (self.f_minsev.get() or 0) > 0:
            where.append("IFNULL(severity,0) >= ?"); args.append(int(self.f_minsev.get()))
        risk = (self.f_risk.get() or "any").strip().lower()
        if risk != "any":
            where.append("LOWER(COALESCE(risk,'')) = ?"); args.append(risk)
    ...
    def load_rows(self) -> None:
        ...
        for r in rows:
            tv.insert("", "end", iid=str(r["_id"]), values=[
                r["date"] or "", r["time"] or "", r["meal"] or "", r["items"] or "",
    ...
    def edit_selected(self) -> None:
        ...
        sel = tv.selection()
        if len(sel) != 1:
            messagebox.showinfo("Health Log", "Select one row to edit.", parent=self.winfo_toplevel()); return
        rid = int(sel[0])
        row = self.db.execute("""
        ...
        self.db.execute("""
            UPDATE health_log
             ...
             WHERE rowid=?
        """, (data["date"], data["time"], data["meal"], data["items"], data["risk"], data["onset_min"],
              data["severity"], data["stool"], data["recipe"], data["symptoms"], data["notes"], rid))
        self.db.commit()
        self.load_rows()
    ...
    # ---------- small edit dialog ----------
    def _edit_dialog(self, row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        dlg = tk.Toplevel(self); dlg.title("Edit Health Entry"); dlg.transient(self.winfo_toplevel()); dlg.grab_set()
        ...
        roww("Meal", ttk.Combobox(frm, textvariable=sv["meal"], values=MEALS, state="readonly", width=12))
        roww("Items", ttk.Entry(frm, textvariable=sv["items"], width=50))
        roww("Risk", ttk.Combobox(frm, textvariable=sv["risk"], values=RISK_OPTS, state="readonly", width=8))
        ...
        roww("Bristol", ttk.Combobox(frm, textvariable=sv["stool_label"], values=BRS_VALUES, state="readonly", width=44))
        roww("Recipe", ttk.Combobox(frm, textvariable=sv["recipe"], values=self._recipe_titles(), width=30))
